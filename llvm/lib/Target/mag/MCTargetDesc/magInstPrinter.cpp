...
#include "magGenAsmWriter.inc"

void magInstPrinter::printRegName(raw_ostream &O, unsigned RegNo) const {
  O << getRegisterName(RegNo);
}

void magInstPrinter::printInst(const MCInst *MI, uint64_t Address,
...
void magInstPrinter::printOperand(const MCInst *MI, int OpNo, raw_ostream &O) {
...
void magInstPrinter::printBranchOperand(const MCInst *MI, uint64_t Address,
...
const char *magInstPrinter::getRegisterName(unsigned RegNo) {
  return getRegisterName(RegNo,
                         ArchRegNames ? mag::NoRegAltName : mag::ABIRegAltName);
}
