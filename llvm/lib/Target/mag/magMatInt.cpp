//===- magMatInt.cpp - Immediate materialisation -------------*- C++ -*--===//
//
...
static int getInstSeqCost(magMatInt::InstSeq &Res, bool HasRVC) {
  if (!HasRVC)
    return Res.size();
...
static void generateInstSeqImpl(int64_t Val,
  ...
  if (isInt<32>(Val)) {
    ...
    if (Hi20)
      Res.push_back(magMatInt::Inst(mag::LUI, Hi20));

    if (Lo12 || Hi20 == 0) {
      unsigned AddiOpc = mag::ADDI;
      Res.push_back(magMatInt::Inst(AddiOpc, Lo12));
    }
    return;
  }
